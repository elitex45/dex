...
contract Dex {
    ...
    function get_drupees_reserve() public view returns (uint256) {
    ...
    function tokenize(
        ...
        require(
            (bytes(digest).length == bytes(signature).length) &&
                (keccak256(bytes(digest)) == keccak256(bytes(signature)))
        );
    ...
    function ethTodrupee() public payable returns (uint256) {
        ...
        uint256 tokens_bought = price(
            msg.value,
            address(this).balance - (msg.value) - eth_dai_liquidity,
            drupees_reserve
        );
    ...
    function ethTodai() public payable returns (uint256) {
        ...
        uint256 tokens_bought = price(
            msg.value,
            address(this).balance - (msg.value) - eth_drupee_liquidity,
            dai_reserve
        );
    ...
    function drupeeToEth(uint256 tokens) public returns (uint256) {
        ...
        uint256 eth_bought = price(
            tokens,
            drupee_reserve,
            address(this).balance - eth_dai_liquidity
        );
    ...
    function daiToEth(uint256 tokens) public returns (uint256) {
        ...
        uint256 eth_bought = price(
            tokens,
            dai_reserve,
            address(this).balance - eth_drupee_liquidity
        );
        (bool sent, ) = msg.sender.call{value: eth_bought}("");
    ...
    function deposit_eth_drupee() public payable returns (uint256) {
        ...
        uint256 drupee_reserve = drupee.balanceOf(address(this));
        uint256 token_amount = (msg.value.mul(drupee_reserve) /
            eth_drupee_liquidity).add(1);
        uint256 liquidity_minted = msg.value.mul(eth_drupee_liquidity) /
            total_eth_derupee;
        eth_drupe_liquidity[msg.sender] = eth_drupe_liquidity[msg.sender].add(
            liquidity_minted
        );
    ...
    function withdraw_eth_drupee(uint256 amount)
        public
        returns (uint256, uint256)
    {
        uint256 drupee_reserve = drupee.balanceOf(address(this));
        uint256 eth_amount = amount.mul(eth_drupee_liquidity) /
            total_eth_derupee;
        uint256 token_amount = amount.mul(drupee_reserve) / total_eth_derupee;
        eth_drupe_liquidity[msg.sender] = eth_drupe_liquidity[msg.sender].sub(
            eth_amount
        );
    ...
    function deposit_eth_dai() public payable returns (uint256) {
        //uint256  eth_drupee_liquidity = address(this).balance.sub(msg.value);
        uint256 dai_reserve = dai.balanceOf(address(this));
        uint256 token_amount = (msg.value.mul(dai_reserve) / eth_dai_liquidity)
            .add(1);
        uint256 liquidity_minted = msg.value.mul(eth_dai_liquidity) /
            total_eth_dai;
        eth_da_liquidity[msg.sender] = eth_da_liquidity[msg.sender].add(
            liquidity_minted
        );
    ...
    function withdraw_eth_dai(uint256 amount)
        public
        returns (uint256, uint256)
    {
        uint256 dai_reserve = dai.balanceOf(address(this));
        ...
        eth_da_liquidity[msg.sender] = eth_da_liquidity[msg.sender].sub(
            eth_amount
        );
